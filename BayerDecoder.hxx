 ...
 */
#ifndef BAYERDECODER_H
#define BAYERDECODER_H
...
public:
                ...
                unsigned rgb_line_step = 0)
  {
    if (width > xn_image.XRes () || height > xn_image.YRes ())
      throw exception("Upsampling only possible for multiple of 2 in both dimensions. Request was %d x %d -> %d x %d.");
    ...
    if (xn_image.XRes () == width && xn_image.YRes () == height)
    {
      ...
      else if (debayering_method_ == EdgeAwareWeighted)
      {
        ...
        throw exception("Unknwon debayering method.");
    }
    else
    {
      if (xn_image.XRes () % width != 0 || xn_image.YRes () % height != 0)
		throw exception("Downsampling only possible for integer scales in both dimensions.");
